...
class QueryDslSortTest {
    ...
    void sortingTest() {
        ...
        //then : 나는 거짓이라고 단언한다. 비워있는게 거짓이라고~
    ...
    void pagingTest() {
        ...
        List<Idol> pagedIdols = factory
                .selectFrom(idol)
                .orderBy(idol.age.desc()) // 나이 상위 2명
                .offset((pageNo - 1) * amount) //1 페이지
                .limit(amount) //2개
                .fetch();
    ...
    void pagingWithJpaTest() {
        ...
        //when
        Page<Idol> pagedIdols = idolRepository.foundAllByPageing(pageInfo);
    ...
    void testSortByNameAsc() {
        ...
        //추가 검증 예시: 첫 번째 아이돌이 이름순으로 올바르게 정렬되었는지
    ...
    void testSortByAgeDescAndPaging() {
        //given
        int pageNumber = 0; //첫번째 페이지
        int pageSize = 3; // 페이지 데이터 수
        ...
        //then
        assertFalse(pagedIdols.isEmpty());

        System.out.println("\n\n\n");
        pagedIdols.forEach(System.out::println);
        System.out.println("\n\n\n");

        //추가 검증 예시: 첫번째 페이지의 첫번째 아이돌이 나이가 가장 많은
    ...
    @Test
    @DisplayName("특정 그룹의 아이돌을 이름 기준으로 오름차순 정렬 및 페이징")
...
//        System.out.println("pagedIdols = " + pagedIdols);
    ...
    void pagingWithJpaTest() {
        //given
        Pageable pageInfo = PageRequest.of(1, 2);
